...
export async function saveArticleContent(
  ...
  const topicLocationMap: TopicLocationMap = newsTopics.topics
    .slice(0, 6)
    .reduce<TopicLocationMap>((map, current) => {
      const input = current.input.find(
        (it) => it.index > newsTopics.title.length
      );
      if (input) {
        map[current.topic.id] = {
          index: input.index - newsTopics.title.length - 1,
