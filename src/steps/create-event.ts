...
export async function createEvent(
  ...
  if (newsWithEvent.length) {
    event = await addNewsToEvent(
      dataService,
      newsWithEvent[0].eventId as string,
      newsItem
    );
...
async function createNewsEvent(
  dataService: DataService,
  imagesStorage: ImagesStorageService,
  newsItems: NewsItem[]
) {
  ...
  const allTopics = newsItems.reduce<Topic[]>(
    (list, item) => list.concat(item.topics || []),
    []
  );
  ...
  const contentItem = await findEventContentItem(
    dataService.articleContentRep,
    newsItems
  );
  if (!contentItem) {
  ...
  let items = newsItems.filter(
    (item) => isValidEventNewsItem(item, title) && item.id !== contentItem.id
  );
  ...
  await dataService.articleContentRep.put(
    ArticleContentBuilder.build({
  ...
  await Promise.all(
    newsItems.map((item) =>
      dataService.newsRep.update({ id: item.id, set: { eventId: event.id } })
    )
  );
...
async function addNewsToEvent(
  ...
  const eventNews = await dataService.newsRep.latestByEvent(
    { eventId, country: event.country, lang: event.lang, limit },
  ...
  if (
    ...
    isValidEventNewsItem(newsItem, event.title) &&
    newsItem.id !== event.source.id
  ) {
...
function isValidEventNewsItem(item: NewsItem, eventTitle: string) {
  return (
    item.title.length > MIN_TITLE_LENGTH &&
    item.title.length < MAX_TITLE_LENGTH &&
    !atonic(item.title.toLowerCase()).startsWith(
      atonic(eventTitle.slice(0, 30).toLowerCase())
    )
  );
}

async function findEventContentItem(
  contentRep: ArticleContentRepository,
  newsItems: NewsItem[]
) {
  ...
  const content = await contentRep.getById(
    ArticleContentBuilder.createId({ refId: contentItem.id, refType: "NEWS" })
  );
  if (!content) {
    return;
  }
...
async function findBestEventImage(
  imageRep: ImageRepository,
  newsItems: NewsItem[]
) {
  const imageItems = newsItems.filter(
    (item) =>
      item.imagesIds &&
  ...
  const ids = uniq(
    imageItems.reduce<string[]>(
      (list, item) => list.concat(item.imagesIds || []),
      []
    )
  );
  ...
  const item = imageItems.find(
    (item) => (item.imagesIds && item.imagesIds.includes(image.id)) || false
  );
  if (!item) {
...
function findBestEventSummary(title: string, newsItems: NewsItem[]) {
  ...
  const data = newsItems
    .map((item) => ({ summary: item.summary, score: search(item.summary) }))
...
function extractEventTopics(
  ...
  const topicsMap = topics.reduce<Dictionary<{ score: number; topic: Topic }>>(
    (map, topic) => {
      if (!map[topic.id]) {
