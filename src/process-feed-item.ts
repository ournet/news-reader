...
import { NewsHelper, Topic } from "@ournet/news-domain";
...
export async function processFeedItem(
  ...
  const newsTopics = await saveNewsTopics(
    dataService.topicRep,
    topicsService,
    newsItem.title,
  ...
  if (newsTopics.topics.length) {
    newsItem.topics = newsTopics.topics.slice(0, 6).map<Topic>((item) => ({
      id: item.topic.id,
  ...
  if (newsData.images.length) {
    for (const image of newsData.images) {
      const savedImage = await saveNewsImage(
        dataService.imageRep,
        imagesStorage,
        image,
        newsData.url,
        newsData.lang
  ...
  const quotesIds = await saveNewsQuotes(
    dataService.quoteRep,
    newsItem,
    newsTopics
  );
  if (quotesIds.length) {
