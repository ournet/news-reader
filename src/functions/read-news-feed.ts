...
export async function readNewsFeed(
  ...
  const lastUrl = await getLastReadedFeedUrl(
    { lang, country: source.country },
    feed.url
  );
  if (lastUrl) {
    const lastUrlIndex = feedReaderItems.findIndex(
      (item) => item.link === lastUrl
    );
    if (lastUrlIndex > -1) {
  ...
  for (const feedItem of feedReaderItems) {
    ...
    const summary = sanitizeNewsText(
      extractTextFromHtml(feedItem.description || feedItem.summary || ""),
      lang
    );

    newsItem.summary = summary && summary.trim();
    if (feedItem.content || feedItem.description) {
      const content = sanitizeNewsText(
        extractTextFromHtml(feedItem.content || feedItem.description || ""),
        lang
      );
      if (content !== summary) {
